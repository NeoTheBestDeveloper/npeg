...
from npeg.bindings import ImgResultStruct, ImgResultType, ImgStruct, ImgType, Interpolation, npeg_core
...
class Img:
    ...
    def write(self, path: str | Path) -> None | NoReturn:
        ...
        if img_result.type == ImgResultType.DO_NOT_HAVE_WRITE_PERMISSIONS:
            raise PermissionError(f"Do not have permissions for writing to '{path}' file.")
        return None
    ...
    def interpolate(self, inter: Interpolation) -> None:
        npeg_core.img_interpolate(self._img_struct, inter)
